...
describe('connectToDB', () => {
  ...
  test('handles unsupported MongoDB version', async () => {
    // Mock an older MongoDB version
    mockAdmin.command.mockResolvedValue({ version: '3.6.0' })

    // Mock connect to only be called once and throw the version error
    mongoose.connect.mockImplementation(async () => {
      // This will cause our code to enter the catch block immediately
      const error = new Error('Unsupported MongoDB version.')
      error.name = 'MongoVersionError'
      throw error
    })

    await expect(connectToDB('mongodb://test', true, 1)).rejects.toThrow(
      'Failed to connect to the database after multiple attempts.'
    )

    expect(devLogger).toHaveBeenCalledWith(
      expect.stringContaining('Unsupported MongoDB version'),
      'error'
    )
  })
  ...
  test('gives up after max retries', async () => {
    ...
    await expect(connectToDB('mongodb://test', true, 2)).rejects.toThrow(
      'Failed to connect to the database after multiple attempts.'
    )
