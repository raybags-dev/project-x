...
axiosInstance.interceptors.response.use(
  (response) => {
    ...
    if (process.env.NODE_ENV === "development") {
      try {
        ...
        devLogger(`Error logging response data: ${error.message}`, "warn");
      }
    }
  ...
  async (error) => {
    ...
    if (error.response) {
      logger(
        `${url}: HTTP ${error.response.status} - ${error.message}`,
        "warn"
      );
    ...
    if (config.__retryCount < maxRetries && isRetryableError(error)) {
      ...
      if (config.__retryCount === maxRetries) {
        ...
        config.proxy = false;
        logger(
          `Final retry attempt using direct connection for ${url}`,
          "warn"
        );
      }
